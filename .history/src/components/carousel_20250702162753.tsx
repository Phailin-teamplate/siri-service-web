...
export default function PortfolioCarousel() {
  ...
  return (
    <div className="w-full max-w-screen-xl mx-auto  md:px-8 py-20 scroll-mt-24" id="portfolio">
      {/* Section Title */}
      ...
      <Carousel className="relative w-full">
            ...
            >
              <div className="flex flex-col md:flex-row items-center justify-between gap-10 md:gap-14">
                ...
                <div className="w-full md:w-1/2">
                  <div className="w-full h-[300px] md:h-[340px] overflow-hidden rounded-xl shadow-md">
                    <Image
                      src={item.image}
                      alt={item.title}
                      width={600}
        ...
        <CarouselPrevious
          ...
          className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white border border-gray-300 rounded-full w-9 h-9 shadow-sm hover:bg-gray-100 transition"
        />
        <CarouselNext
          onClick={() => setActiveIndex((prev) => (prev + 1) % items.length)}
          className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white border border-gray-300 rounded-full w-9 h-9 shadow-sm hover:bg-gray-100 transition"
        />
