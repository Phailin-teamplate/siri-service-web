      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 sm:gap-8 md:gap-10 place-items-center">
        {[KLab, Ktra, AKCF, KWISA, FEN, IDCC, NIPA, KCFS, DOP, BPICT].map((logo, index) => (
          <div key={index} className="flex items-center justify-center w-full">
            <div className="w-50 h-50 sm:w-28 sm:h-28 md:w-32 md:h-50 lg:w-50 lg:h-50 xl:w-50 xl:h-50 2xl:w-50 2xl:h-50  flex items-center justify-center overflow-hidden">
              <Image
                src={logo}
                alt={`Logo ${index + 1}`}
                width={250}
