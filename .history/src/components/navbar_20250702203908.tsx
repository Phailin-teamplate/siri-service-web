...
export default function Navbar() {
  ...
  useEffect(() => {
    const sectionIds = ["about", "contact", "customer", "portfolio"];
    ...
    >
      <nav className="flex items-center justify-between px-4 sm:px-6 lg:px-8 py-3">
        ...
        <div className="hidden md:flex gap-4 lg:gap-6">
          {["#about", "#contact", "#customer", "#portfolio"].map((id, i) => (
            <button
              key={id}
        ...
        ${
          activeSection === id
            ...
            >
              {["About", "contact", "Customers", "Portfolio"][i]}
            </button>
          ))}
        </div>
        {/* Mobile Menu Button */}
        ...
        <div className="flex flex-col space-y-4 mt-4">
          {["#about", "#contact", "#customer", "#portfolio"].map((id, i) => (
            <button
              key={id}
              onClick={() => handleSmoothScroll(id)}
              className={`text-left text-lg font-medium px-3 py-2 rounded-md transition-all duration-300
        ${
          activeSection === id
            ? "bg-blue-600 text-white shadow-md"
            : "text-gray-800 hover:bg-blue-100 hover:text-blue-600"
